#!/usr/bin/env python3
"""
GitHub Repository Secrets Batch Update Script
Adds/updates Secrets across all repositories in bulk.
"""
...
# ==================== Configuration ====================
GITHUB_TOKEN = os.environ.get('GITHUB_TOKEN', '')  # Or enter directly
...
# Dry-run mode (if True, no actual updates will be made)
DRY_RUN = False
...
def get_all_repos(org: str, token: str):
    ...
    while True:
        ...
        # If it's a personal account, not an Organization:
        # url = f'https://api.github.com/users/{org}/repos?per_page=100&page={page}'
        
        response = requests.get(url, headers=headers)
        
        if response.status_code == 404:
            # If not an Organization, try personal repositories
            url = f'https://api.github.com/users/{org}/repos?per_page=100&page={page}'
            response = requests.get(url, headers=headers)
        ...
        print(f"  Page {page}: {len(data)} found (cumulative: {len(repos)})")
        page += 1
...
def update_secret(org: str, repo: str, secret_name: str, secret_value: str, token: str, dry_run=False):
    ...
    try:
        ...
        if key_response.status_code != 200:
            return False, f'Failed to fetch public key: HTTP {key_response.status_code}', action
...
def main():
    ...
    if not GITHUB_TOKEN:
        ...
        print("Set it as an environment variable or enter it directly in the script:")
    ...
    # Confirmation
    if not DRY_RUN:
        print(f"\n{Colors.YELLOW}Are you sure you want to update {SECRET_NAME} for all repositories?{Colors.RESET}")
    ...
    print_success(f"Found {len(repos)} repositories in total")
    ...
    # Results summary
    print_header("3. Results Summary")
