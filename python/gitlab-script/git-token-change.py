...
모든 리포지토리에 Secret을 일괄적으로 추가/업데이트합니다.
"""
...
GITHUB_ORG = ''  # Organization 이름 또는 username
SECRET_NAME = 'GITLAB_TOKEN'  # 업데이트할 Secret 이름
SECRET_VALUE = os.environ.get('SECRET_VALUE', '')  # Secret 값 (환경변수 권장)
...
def main():
    ...
    if not SECRET_VALUE:
        print_error("SECRET_VALUE가 설정되지 않았습니다!")
        print("환경변수로 설정하거나 스크립트에 직접 입력해주세요:")
