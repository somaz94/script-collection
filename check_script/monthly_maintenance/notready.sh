...
if [ "${STATE}" == "NotReady" ];then
    touch /home/somaz/monthly_maintenance/send_mail
    ...
    sudo cat /home/somaz/monthly_maintenance/send_mail | sudo mail -s "$(hostname) Node NotReady" ${recipient}
    sudo rm -f /home/somaz/monthly_maintenance/send_mail
    exit 0
    echo ""
    exit 0
fi
